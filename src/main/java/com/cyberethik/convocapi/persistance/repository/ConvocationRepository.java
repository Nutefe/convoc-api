...
public interface ConvocationRepository extends JpaRepository<Convocations, Long> {
    ...
    @Query("SELECT x FROM Convocations x WHERE x.deleted = false AND x.evenement.id IN :events")
    ...
    @Query("SELECT COUNT(x) FROM Convocations x WHERE x.deleted = false AND x.evenement.id IN :events")
