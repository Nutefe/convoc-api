...
public interface EquipeRepository extends JpaRepository<Equipes, Long> {
    ...
    @Query("SELECT x FROM Equipes x WHERE x.deleted = false AND x.organisation.id IN :orgs")
    ...
    @Query("SELECT COUNT(x) FROM Equipes x WHERE x.deleted = false AND x.organisation.id IN :orgs")
